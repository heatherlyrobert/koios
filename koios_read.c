 ...
 *>    /+---(defense)------------------------+/                                                                                                                                          <* 
 *>    UDEBUG_KOIOS   ylog_upoint  ("a_dir"     , a_dir);                                                                                                                                  <* 
 *>    --rce;  if (a_dir   == NULL) {                                                                                                                                                    <* 
 ...
 *>       return rce;                                                                                                                                                                    <* 
 *>    }                                                                                                                                                                                 <* 
 *>    UDEBUG_KOIOS   ylog_uinfo   ("a_dir"     , a_dir);                                                                                                                                  <* 
 ...
 *>    UDEBUG_KOIOS   ylog_upoint  ("r_file"    , r_file);                                                                                                                                 <* 
 *>    --rce;  if (r_file  == NULL) {                                                                                                                                                    <* 
 ...
 *>    }                                                                                                                                                                                 <* 
 *>    /+---(mode)---------------------------+/                                                                                                                                          <* 
 *>    UDEBUG_KOIOS   ylog_uchar   ("a_mode"     , a_mode);                                                                                                                                <* 
 *>    --rce;  switch (a_mode) {                                                                                                                                                         <* 
 *>    case 'r'  :                                                                                                                                                                       <* 
 *>       strcpy (x_mode, "rt");                                                                                                                                                         <* 
 *>       break;                                                                                                                                                                         <* 
 ...
 *>       break;                                                                                                                                                                         <* 
 *>    }                                                                                                                                                                                 <* 
 *>    UDEBUG_KOIOS   ylog_uinfo   ("x_mode"    , x_mode);                                                                                                                                 <* 
 *>    /+---(full name)----------------------+/                                                                                                                                          <* 
 *>    l = strlen (a_dir);                                                                                                                                                               <* 
 ...
 *>    UDEBUG_KOIOS   ylog_upoint  ("f"         , f);                                                                                                                                      <* 
 *>    --rce;  if (f == NULL) {                                                                                                                                                          <* 
 *>       yerr_uerror ("%s/%s:%d:0: error: file å%sæ could not be found/openned for (%c)", a_sfile, a_sfunc, a_sline, x_name, a_mode);                                                   <* 
 *>       UDEBUG_KOIOS   ylog_unote   ("can not open script file");                                                                                                                        <* 
 *>       UDEBUG_KOIOS   ylog_uexitr  (__FUNCTION__, rce);                                                                                                                                 <* 
 *>       return rce;                                                                                                                                                                    <* 
 *>    }                                                                                                                                                                                 <* 
 *>    UDEBUG_KOIOS   ylog_unote   ("script file open");                                                                                                                                   <* 
 *>    /+---(save-back)----------------------+/                                                                                                                                          <* 
 ...
 *>    UDEBUG_KOIOS   ylog_uenter  (__FUNCTION__);                                                                                        <* 
 *>    /+---(defense)------------------------+/                                                                                         <* 
 *>    UDEBUG_KOIOS   ylog_upoint  ("b_file"    , b_file);                                                                                <* 
 *>    --rce;  if (b_file  == NULL) {                                                                                                   <* 
 *>       yerr_uerror ("%s/%s:%d:0: error: file å%sæ passed with NULL file pointer return", a_sfile, a_sfunc, a_sline, a_name);         <* 
 *>       UDEBUG_KOIOS   ylog_uexitr  (__FUNCTION__, rce);                                                                                <* 
 *>       return rce;                                                                                                                   <* 
 ...
 *>    fflush (*b_file);                                                                                                                <* 
 *>    rc = fclose (*b_file);                                                                                                           <* 
 *>    UDEBUG_KOIOS   ylog_uvalue  ("rc"        , rc);                                                                                    <* 
 *>    --rce;  if (rc < 0) {                                                                                                            <* 
...
READ__defense           (FILE **a_scrp, int *r_nline, char *r_dittoing, char *r_dmark, int *r_ditto, int *r_dline, int *r_nrecd, char r_recd [LEN_RECD])
{
   ...
   UDEBUG_KOIOS   ylog_upoint  ("r_nrecd"   , r_nrecd);
   --rce;  if (r_nrecd == NULL) {
      UDEBUG_KOIOS   ylog_uexitr  (__FUNCTION__, rce);
      return rce;
   }
   UDEBUG_KOIOS   ylog_upoint  ("r_recd"    , r_recd);
   --rce;  if (r_recd == NULL) {
      UDEBUG_KOIOS   ylog_uexitr  (__FUNCTION__, rce);
      return rce;
   }
...
READ__single            (FILE **b_scrp, int *r_nline, char *r_dittoing, char *r_dmark, int *r_ditto, int *r_dline, int *r_nrecd, char r_recd [LEN_RECD])
{
   ...
   /*---(read next)-------------------*/
   UDEBUG_KOIOS   ylog_unote   ("read script file");
   fgets (x_recd, LEN_RECD, *b_scrp);
   if (feof (*b_scrp)) {
      UDEBUG_KOIOS   ylog_unote   ("hit end of file");
      UDEBUG_KOIOS   ylog_uexit   (__FUNCTION__);
      return 0;
   }
   /*---(handle line numbers)---------*/
   UDEBUG_KOIOS   ylog_ucomplex("line#"     , "%4d nline, %c, %4d dline", *r_nline, *r_dittoing, *r_dline);
   rc = DITTO_read_numbering (*r_dittoing, *r_ditto, r_nline, r_dline);
   UDEBUG_KOIOS   ylog_uvalue  ("ditto prep", rc);
   if (rc == 2) {
      UDEBUG_KOIOS   ylog_unote   ("quickly to next record");
   ...
   UDEBUG_KOIOS   ylog_ucomplex("x_recd"    , "%3då%sæ", x_len, x_recd);
   /*---(filter)----------------------*/
   if (x_recd [0] == '\0') {
      UDEBUG_KOIOS   ylog_unote   ("SKIP, empty");
      if (*r_dittoing == 'y')  DITTO_end (b_scrp, r_dittoing, r_dmark, r_ditto, r_dline);
      UDEBUG_KOIOS   ylog_uexitr  (__FUNCTION__, 2);
      return 2;
   }
   if (x_len > 1 && x_recd [0] == '#' && x_recd [1] != '>') {
      UDEBUG_KOIOS   ylog_unote   ("SKIP, comment");
      if (*r_dittoing == 'y')  DITTO_end (b_scrp, r_dittoing, r_dmark, r_ditto, r_dline);
      UDEBUG_KOIOS   ylog_uexitr  (__FUNCTION__, 2);
      return 2;
   }
   UDEBUG_KOIOS   ylog_uvalue  ("length"    , x_len);
   if (x_len <=  5 && strncmp (x_recd, "#>", 2) != 0)  {
      UDEBUG_KOIOS   ylog_unote   ("SKIP, too short");
      if (*r_dittoing == 'y')  DITTO_end (b_scrp, r_dittoing, r_dmark, r_ditto, r_dline);
      UDEBUG_KOIOS   ylog_uexitr  (__FUNCTION__, 2);
      return 2;
   }
   ...
   if (*r_dittoing == 'y' && *r_dline != *r_ditto)  {
      UDEBUG_KOIOS   ylog_unote   ("in dittoing logic");
      ...
      if (p != NULL) {
         ...
         rc = VERB_dittoable (p);
         UDEBUG_KOIOS   ylog_uvalue  ("dittoable" , rc);
         if (rc != 1) {
   ...
   /*---(save-back)-------------------*/
   UDEBUG_KOIOS   ylog_uinfo   ("x_recd"    , x_recd);
   UDEBUG_KOIOS   ylog_unote   ("save copy of source record");
...
READ_next               (FILE **b_scrp, int *r_nline, char *r_dittoing, char *r_dmark, int *r_ditto, int *r_dline, int *r_nrecd, char r_recd [LEN_RECD])
{
   ...
   /*---(prepare)------------------------*/
   /*> READ__clear  ();                                                               <*/
   UDEBUG_KOIOS   ylog_uvalue  ("feof"      , feof (*b_scrp));
   if (feof (*b_scrp)) {
      UDEBUG_KOIOS   ylog_unote   ("already at end of file");
      UDEBUG_KOIOS   ylog_uexit   (__FUNCTION__);
      return 0;
   }
   /*---(read for a good record)---------*/
   UDEBUG_KOIOS   ylog_unote   ("read loop");
   while (1) {
      rc = READ__single (b_scrp, r_nline, r_dittoing, r_dmark, r_ditto, r_dline, r_nrecd, r_recd);
      UDEBUG_KOIOS   ylog_uvalue  ("single"    , rc);
      if (rc <= 0) {
         UDEBUG_KOIOS   ylog_unote   ("reached end of file");
         UDEBUG_KOIOS   ylog_uexit   (__FUNCTION__);
         return 0;
      }
      if (rc == 2)  continue;
      UDEBUG_KOIOS   ylog_unote   ("got a good record");
      break;
   }
   UDEBUG_KOIOS   ylog_uinfo   ("r_recd"    , r_recd);
   /*---(complete)-----------------------*/
